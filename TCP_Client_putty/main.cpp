...
void main()
{
	...
	/*	getnameinfo func - 
		~  host ко 접속하고자 하는 computer의 이름을 저장
		           service ке 접속할 소켓에 랜덤으로 주어진 포트번호를 저장..
			   
		--> 만일 다 된다면 returns 0 == succeed :   80 - LAPTOP_name connected on port 2275
	*/
		...
		/*
			...
			puts received data to the buf, then returns  byte size of that received data
					...
					ex: received data == input data : "Zarina"  --> buf = Hello
															 	    return = 5 bytes
					      (echo ) send - clientSocket으로 buf내용을 전송하라
		*/
